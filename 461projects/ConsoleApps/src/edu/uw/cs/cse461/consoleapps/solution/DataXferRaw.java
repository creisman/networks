 ...
 * 
 */
public class DataXferRaw extends NetLoadableConsoleApp implements
		DataXferRawInterface {
	...
	/**
	 * This method is invoked each time the infrastructure is asked to launch
	 * this application.
	 */
	@Override
	public void run() {
		...
		try {
			...
			for (int index = 0; index < DataXferRawService.NPORTS; index++) {
				...
				TransferRateInterval udpStats = udpDataXferRate(
						DataXferServiceBase.HEADER_BYTES, server, port,
						socketTimeout, xferLength, nTrials);

				System.out.println("UDP: xfer rate = "
						+ String.format("%9.0f", udpStats.mean() * 1000.0)
						+ " bytes/sec.");
				System.out.println("UDP: failure rate = "
						+ String.format("%5.1f", udpStats.failureRate()) + " ["
				...
				TransferRateInterval tcpStats = tcpDataXferRate(
						DataXferServiceBase.HEADER_BYTES, server, port,
						socketTimeout, xferLength, nTrials);
	...
	/**
	 * This method performs the actual data transfer, returning the result. It
	 * doesn't measure performance, though.
	 * 
	 * @param header
	 *            The header to put on the outgoing packet
	 * @param hostIP
	 ...
	 * @param xferLength
	 *            The number of data bytes each response packet should carry
	 */
	@Override
	public byte[] udpDataXfer(byte[] header, String hostIP, int udpPort,
			int socketTimeout, int xferLength) throws IOException {
		// Allocate space for the response
		byte[] responseBuf = new byte[xferLength];

		DatagramSocket socket = new DatagramSocket();
		socket.setSoTimeout(socketTimeout); // wait at most a bounded time when
											// receiving on this socket

		ByteBuffer bufBB = ByteBuffer.wrap(header);
		bufBB.put(header);
		DatagramPacket packet = new DatagramPacket(header, header.length,
				new InetSocketAddress(hostIP, udpPort));
		// Inform the server that we are here, and would like a response
		socket.send(packet);

		// We expect to receive a response packet containing at most 1000
		// payload bytes
		...
		int read = 0;
		try {
			// Keep listening for packets until the buffer overflows, or until
			// the server sends a short packet
			do {
				socket.receive(receivePacket);
				String rcvdHeader = new String(receiveBuf, 0,
						DataXferRawService.RESPONSE_OKAY_LEN);
				if (!rcvdHeader
						.equalsIgnoreCase(DataXferRawService.RESPONSE_OKAY_STR))
					throw new IOException("Bad returned header: got '"
							+ rcvdHeader + "' but wanted '"
							+ DataXferRawService.RESPONSE_OKAY_STR);

				// Calculate the length and starting index of the payload
				int payloadLen = receivePacket.getLength()
						- DataXferRawService.RESPONSE_OKAY_LEN;
				...
				if (payloadLen + read > xferLength)
					throw new IOException("Bad response length: got "
							+ payloadLen + read + "bytes but expected "
							+ xferLength + " bytes.");

				// Used to test our server implementation only.
				// if (receivePacket.getData()[payloadStart] != (byte)1 ||
				// receivePacket.getData()[receivePacket.getLength() - 1] !=
				// (byte)1) {
			...
			} while (receivePacket.getLength() == packetLen
					&& read < xferLength);
			// Verify that the correct number of packets were read
			if (read != xferLength)
				throw new IOException("Bad response length: got " + read
						+ "bytes but expected " + xferLength + " bytes.");
	...
	/**
	 * Performs nTrials trials via UDP of a data xfer to host hostIP on port
	 * udpPort. Expects to get xferLength bytes in total from that host/port. Is
	 * willing to wait up to socketTimeout msec. for new data to arrive.
	 * 
	 * @return A TransferRateInterval object that measured the total bytes of
	 *         data received over all trials and the total time taken. The
	 *         measured time should include socket creation time.
	 */
	@Override
	public TransferRateInterval udpDataXferRate(byte[] header, String hostIP,
			int udpPort, int socketTimeout, int xferLength, int nTrials) {
		...
		for (int trial = 0; trial < nTrials; trial++) {
			try {
				...
				TransferRate.abort("udp", xferLength);
				// System.out.println("Unexpected " + e.getClass().getName() +
				// " exception in UDP trial: " + e.getMessage());
			}
		}
	 ...
	 */
	@Override
	public byte[] tcpDataXfer(byte[] header, String hostIP, int tcpPort,
			int socketTimeout, int xferLength) throws IOException {
		// Allocate space for the response
		byte[] responseBuf = new byte[xferLength];
		...
		try {
			...
			if (len != responseHeader.length)
				throw new IOException("Bad response header length: got " + len
						+ " but expected " + responseHeader.length);

			// Verify the contents of the header
			String headerStr = new String(responseHeader);
			if (!headerStr
					.equalsIgnoreCase(DataXferRawService.RESPONSE_OKAY_STR))
				throw new IOException("Bad response header: got '" + headerStr
						+ "' but expected '"
						+ DataXferRawService.RESPONSE_OKAY_STR + "'");

			// Read fixed-size chunks from the input stream (as per the spec)
			while (read < xferLength) {
				int res = is.read(responseBuf, read,
						Math.min(1000, xferLength - read));
	 ...
	 */
	@Override
	public TransferRateInterval tcpDataXferRate(byte[] header, String hostIP,
			int tcpPort, int socketTimeout, int xferLength, int nTrials) {
