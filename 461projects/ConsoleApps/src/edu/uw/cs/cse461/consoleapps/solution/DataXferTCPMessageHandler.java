...
public class DataXferTCPMessageHandler extends NetLoadableConsoleApp implements
        ...
        try {
            ...
            try {
                ...
                while (true) {
                    ...
                    if (targetTCPPort != 0) {
                        TransferRate.clear();
                        tcpResult = DataXferRate(DataXferServiceBase.HEADER_STR, targetIP, targetTCPPort,
                                socketTimeout, size, nTrials);
                    }

                    if (tcpResult != null) {
                        // Print out the rate in seconds (not ms)
                        System.out.println(String.format("TCP: xfer rate =\t%.2f bytes/sec.", tcpResult.mean() * 1000));
                        System.out.println(String.format("TCP: failure rate =\t%.1f [%d/%d]", tcpResult.failureRate(),
                                tcpResult.nAborted(), tcpResult.nTrials()));
                    }
                }
            ...
            System.out.println("DataXferTCPMessageHandler.run() caught exception: " + e.getMessage());
        }
    }

    /**
     * @see edu.uw.cs.cse461.consoleapps.DataXferInterface.DataXferTCPMessageHandlerInterface#DataXfer(java.lang.String,
     *      java.lang.String, int, int, int)
     */
    @Override
    public byte[] DataXfer(String header, String hostIP, int port, int timeout, int xferLength) throws JSONException,
            IOException {
        ...
        try {
            ...
            if (!response.equalsIgnoreCase(DataXferServiceBase.RESPONSE_OKAY_STR)) {
                throw new IOException("Bad response header: got '" + response + "' but expected '"
    ...
    /**
     * @see edu.uw.cs.cse461.consoleapps.DataXferInterface.DataXferTCPMessageHandlerInterface#DataXferRate(java.lang.String,
     *      java.lang.String, int, int, int, int)
     */
    @Override
    public TransferRateInterval DataXferRate(String header, String hostIP, int port, int timeout, int xferLength,
            int nTrials) {
