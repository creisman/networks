...
/**
 * A simple service that simply echoes back whatever it is sent. It exposes a single method via RPC: echo.
 * <p>
 ...
 * <ol>
 * <li><tt>private RPCCallableMethod<EchoService> echo;</tt> <br>
 * declares a variable that can hold a method description of the type the infrastructure requires to invoke a method.
 * <li><tt>echo = new RPCCallableMethod<EchoService>(this, "_echo");</tt> <br>
 * initializes that variable. The arguments mean that the method to invoke is <tt>this->_echo()</tt>.
 * </ol>
 * <p>
 * <li>Register the method with the RPC service: <br>
 * <tt>((RPCService)OS.getService("rpc")).registerHandler(servicename(), "echo", echo );</tt> <br>
 * This means that when an incoming RPC specifies service "echo" (the 1st argument) and method "echo" (the 2nd), that
 * the method described by RPCCallableMethod variable <tt>echo</tt> should be invoked.
 * </ol>
 * 
 * @author zahorjan
 * 
...
public class EchoRPCService extends EchoServiceBase {
    ...
    /**
     * Key used for EchoRPC's header, in the args of an RPC call. The header element is a string
     * (EchoServiceBase.HEADER_STR).
     */
    ...
    private final RPCCallableMethod echo;
    ...
    public JSONObject _echo(JSONObject args) throws Exception {
        ...
        if (header == null || !header.has(HEADER_TAG_KEY)
                || !header.getString(HEADER_TAG_KEY).equalsIgnoreCase(HEADER_STR)) {
